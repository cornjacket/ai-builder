...
containing `Purpose:` and `Tags:` lines. Use `Tags: implementation, <component-name>`.
...
- Present tense: "Provides..." not "This package provides..."
...
When you write any companion `.md` file, use a Purpose:/Tags: header block at
the top, with `Tags: implementation, <component-name>`:

```
Purpose: First sentence — a standalone, complete description.
...
Purpose. Without it CommonMark renders them on the same visual line.

Do not cite specific tool/language version numbers unless stating a minimum required
version constraint. The source of truth for versions is always a config file
