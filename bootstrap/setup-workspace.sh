...
echo "[1/5] Creating workspace directory..."
mkdir -p "$WORKSPACE"

echo "[2/5] Bare-cloning from bootstrap clone (local, fast)..."
git clone --bare "$BOOTSTRAP_DIR" "$WORKSPACE/.bare"

echo "[3/5] Reconfiguring remote to point to GitHub..."
...
echo "[4/5] Writing .git pointer file..."
echo "gitdir: ./.bare" > "$WORKSPACE/.git"

echo "[5/5] Adding main worktree..."
