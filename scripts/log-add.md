...
Add an entry to [`log.md`](../log.md), or back-fill the most recent
`_pending_` hash with the current short HEAD. Does not commit.
...
formats the entry per the [Work Log](../CLAUDE.md#work-log) convention, and
appends it to `log.md` with `_pending_` as the hash placeholder.
...
1. Append the entry **before** committing the work it describes:
   ```bash
   scripts/log-add.sh --task 14fb29-add-work-log-mechanism -- "Did the thing."
   ```
