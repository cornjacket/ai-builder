...
- **2026-05-03** — Added a 'Standard task workflow' recipe to CLAUDE.md's Development Workflow section: an end-to-end numbered loop (pick → start → align → implement → docs → push → close → tear down) referencing existing scripts. Pins the parallel-implementation playbook in one place. Task: `64aa37-document-standard-task-workflow`. Subtask: `64aa37-0000-add-recipe-to-claude-md`. Commit: `_pending_`.
