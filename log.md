...
- **2026-05-01** — Resolved overlap between log.md and project/status/. Reframed status as a delta document covering the period since the previous report (operator-driven cadence, not daily); replaced the 'sign off' semantic with 'write a status report'. Updated CLAUDE.md, project/README.md, project/status/README.md, and the top-level README pointer. Task: `9d3bc1-resolve-log-md-vs-status-overlap`. Commit: `58f932e`.
- **2026-05-02** — Added --category <branch> filter to list-tasks.sh: get_category and matches_category helpers, applied at depth-1 in print_status_tasks and print_dir_tasks. 'unclassified' matches Category '—' or missing. Task: `8ec43a-list-tasks-by-category`. Subtask: `8ec43a-0000-add-category-filter-flag`. Commit: `485a167`.
- **2026-05-02** — Added --group-by-category to list-tasks.sh: tasks within each status folder are now grouped by Category in canonical class order (gemini-compat, orchestrator-core, ..., workspace-mgmt, unclassified). Combines with --sort-priority and --category. Task: `8ec43a-list-tasks-by-category`. Subtask: `8ec43a-0001-add-group-by-category-flag`. Commit: `cf63553`.
- **2026-05-02** — Added 9 bats tests in tests/unit/shell/test_list_tasks.bats covering --category (match, unclassified, unknown, +sort-priority), --group-by-category (canonical class order, unclassified last, +sort-priority), the --category × --group-by-category combination, and that default behaviour is unchanged. All 49 shell tests pass. Task: `8ec43a-list-tasks-by-category`. Subtask: `8ec43a-0002-add-bats-tests`. Commit: `352e759`.
- **2026-05-02** — Documented --category and --group-by-category: added a 'partition by worktree class' block to ai-builder/CLAUDE.md, updated the script-list flag surface, added project/tasks/README.md examples, and created the companion project/tasks/scripts/list-tasks.md following documentation-standards.md. Task: `8ec43a-list-tasks-by-category`. Subtask: `8ec43a-0003-update-claude-md-and-companion-doc`. Commit: `09900e2`.
- **2026-05-02** — Closed 8ec43a-list-tasks-by-category. Added --category <branch> and --group-by-category to list-tasks.sh, with 9 new bats tests and full CLAUDE.md / companion-doc coverage. Unblocks parallel implementation: 'list-tasks.sh --epic main --folder backlog --group-by-category --sort-priority' is now the canonical 'what's next per worktree class' command. Task: `8ec43a-list-tasks-by-category`. Commit: `b615538`.
- **2026-05-03** — Added a 'Standard task workflow' recipe to CLAUDE.md's Development Workflow section: an end-to-end numbered loop (pick → start → align → implement → docs → push → close → tear down) referencing existing scripts. Pins the parallel-implementation playbook in one place. Task: `64aa37-document-standard-task-workflow`. Subtask: `64aa37-0000-add-recipe-to-claude-md`. Commit: `f49cf50`.
- **2026-05-03** — Closed 64aa37-document-standard-task-workflow. Standard-task-workflow recipe is now in CLAUDE.md as the operator-facing playbook for parallel implementation across worktrees. Task: `64aa37-document-standard-task-workflow`. Commit: `_pending_`.
