...
2. Run `target/setup-project.sh` and verify the directory structure is correct
3. Run `target/init-claude-md.sh` and verify CLAUDE.md and GEMINI.md symlink
   are created
4. Run `target/init-claude-md.sh` again and verify it is idempotent
...
All steps should be scripted in `target/test-setup.sh` so the test is
repeatable.
